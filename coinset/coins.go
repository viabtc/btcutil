...
func (cs *CoinSet) TotalValue() (value dogeutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue dogeutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue dogeutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount dogeutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue dogeutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue dogeutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue dogeutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue dogeutil.Amount, coins []Coin) (Coins, error) {
...
// dogeutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() dogeutil.Amount {
	return dogeutil.Amount(c.txOut().Value)
}
